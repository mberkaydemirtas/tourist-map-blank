...
export default function createAsyncStorageDriver() {
  return {
    async listTrips() {
      ...
      return items.filter(t => !t.deletedAt).sort((a,b) => (b.updatedAt||0) - (a.updatedAt||0));
    },
    async getTrip(id) {
      const items = await readAll();
    ...
    async createTrip(init = {}) {
      ...
      const trip = emptyTrip({ ...init, title: numbered });
    ...
    async updateTrip(id, patch, expectedVersion) {
      ...
      const updated = { ...current, ...patch, version: (current.version||1)+1, updatedAt: now() };
      items[idx] = updated;
    ...
    async deleteTrip(id) {
      ...
      const idx = items.findIndex(t => t.id === id);
      if (idx === -1) return;
      items[idx] = { ...items[idx], deletedAt: now(), updatedAt: now(), version: (items[idx].version||1)+1 };
      await writeAll(items);
    },
    async duplicateTrip(id) {
      ...
      const t = now();
      const copy = { ...src, id: `${t}_${Math.random().toString(36).slice(2,6)}`, title: `Kopya - ${src.title}`, createdAt: t, updatedAt: t, version: 1 };
