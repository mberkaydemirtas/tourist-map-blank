...
export function normalizeRaw(item) {
  ...
  const coords = Number.isFinite(lat) && Number.isFinite(lon) ? { lat, lng: lon } : null;

  const category = item.category || item.amenity || item.tourism || item.shop || 'sights';

  return {
    id: item.id || item.osm_id || item.place_id || `${item.source || 'x'}:${item.name || ''}:${lat},${lon}`,
...
export async function resolvePlacesBatch({ items, city = '' }) {
      ...
      for (const x of normalized) {
        ...
        if (m?.matched && m.place_id) {
          ...
          if (Number.isFinite(gLat) && Number.isFinite(gLon)) {
            x._google_coords = { g_lat: gLat, g_lon: gLon };
            x.coords = { lat: gLat, lng: gLon };
            x.lat = x.coords.lat;
            x.lon = x.coords.lng;
          }
  ...
  // 2) Google fallback — only for cache misses
  const unresolved = normalized.filter(
    (x) => !x.place_id && x.coords && x.name && String(x.name).trim().length > 1
  );
